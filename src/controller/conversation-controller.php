<?php 
    ...
    // In the send_message action, add notification for new messages
    ...
    // After successfully inserting the message, send a notification to the recipient
    $sender_id = $_SESSION['userid'];
    $sender_name = $_SESSION['fullname'];
    
    // Get the recipient ID (the other user in the conversation)
    $recipientQuery = "SELECT user_id FROM tbl_conversation WHERE conversation_id = :conversation_id AND user_id != :sender_id";
    $stmt = $pdo->prepare($recipientQuery);
    ...
    if ($recipient) {
        $notificationMessage = "You have received a new message from {$sender_name}.";
        $createNotification->insert($recipient['user_id'], $notificationMessage);
    }
